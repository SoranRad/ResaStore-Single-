...
{
    public partial class FormListIncome : Form
    {
        #region Logging
        ...
        private void RefreshGrid        ()
        {
            try
            {
                mS_GridX1.DataSource = _Manager
                    .GetList<Accounts>(new { Kind = Enums.NzAccountKind.Income })?
                    .ToList();
            }
            catch (Exception ex)
            {
        ...
        private void Create_Form        (Accounts Income)
        {
            _FormIncome?.Dispose();
            _FormIncome              = new FormIncome(_Manager,Income);
        ...
        private void FormListIncome_Load            (object sender, EventArgs e)
        {
            RefreshGrid();
        }
