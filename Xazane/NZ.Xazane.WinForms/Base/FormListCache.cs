...
{
    public partial class FormListCache : Form
    {
        #region Logging
        ...
        private Form_Cache     _formCache;
        private Manager        _Manager;
        #endregion
        #region Constructor
        public FormListCache       ()
        {
            InitializeComponent();
        ...
        private void RefreshGrid            ()
        {
            try
            {
                mS_GridX1.DataSource    = _Manager
                    .GetList<Accounts>(new { Kind = Enums.NzAccountKind.Cache })?
                    .ToList();
            }
            catch (Exception ex)
            {
        ...
        private void Create_Form            (Accounts Cache)
        {
            _formCache?.Dispose();

            _formCache              = new Form_Cache(_Manager,Cache);
            _formCache.MS_Do_Save  += Frm_MS_Do_Save;
            _formCache.FormClosed  += Frm_FormClosed;
        }
        ...
        {
            Create_Form(null);
            _formCache.Show(this);
        }
        private void FormListSanduq_Load            (object sender, EventArgs e)
        {
            RefreshGrid();
        }
