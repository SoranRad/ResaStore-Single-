...
{
    public partial class FormListPayment : Form
    {
        #region Logging
        ...
        #endregion
        public FormListPayment()
        {
            InitializeComponent();
            this.Icon   = global::MS_Resource.GlobalResources.Logo_Resaa;
            _Manager    = new DpManager();
            SetCurrentMonth();
            NzKinds.SelectedTabChanged  += NzFactorKinds_SelectedTabChanged;
        ...
        private void RefreshGrid                        ()
        {
            try
            {
                ...
                var list =
                    _Manager.GetView<PaymentList>(new
                    {
        ...
        private void RefreshItem                        ()
        {
            if(NzGridHeads.CurrentRow==null || NzGridHeads.CurrentRow.RowType!=RowType.Record)
                return;
            if(NzGridHeads.CurrentRow.DataRow is PaymentList row)
            {
                try
                {
        ...
        private void Edit                               ()
        {
            ...
            //new FormPayment(row.ID, _Kind).ShowDialog(this);
            else 
                return;
        ...
        private void Print                              (Enums.NzKindPrint PrintKind)
        {
            ...
            new Print(ListIDs,PrintKind, _Kind).Show(this);
        }
        private void Create_Form                        (PaymentList Pay)
        {
            _Form?.Dispose();
            _Form = new FormPayment(Pay?.ID??0, _Kind);
        ...
        private void Frm_MS_Do_Save                     (object sender, EventArgs e)
        {
            ...
            var row = NzGridHeads.GetRows()
                .SingleOrDefault(x => (x.DataRow as PaymentList).ID == id);
            if (row == null) return;
        ...
        private void FormListPayment_Shown              (object sender, EventArgs e)
        {
            RefreshGrid();
        }
