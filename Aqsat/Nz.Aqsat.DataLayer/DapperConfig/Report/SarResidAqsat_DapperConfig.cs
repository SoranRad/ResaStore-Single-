...
{
    public class SarResidAqsat_DapperConfig : DapperEntityConfiguration<SarResidAqsat>
    {
	    public SarResidAqsat_DapperConfig()
	    {
		    SetList(@"
...
WHERE   
            ...
            tar.isPardaxt = 0 
        AND DATEDIFF(DAY,GETDATE(),tar.tarixQest) >= 0 
        AND DATEDIFF(DAY,GETDATE(),tar.tarixQest) <= @ruz
