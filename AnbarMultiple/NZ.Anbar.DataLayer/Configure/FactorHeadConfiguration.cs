...
namespace NZ.Anbar.DataLayer.Configure
{
    public class FactorHeadConfiguration : EntityTypeConfiguration<FactorHead>
    {
        public FactorHeadConfiguration()
        {
            ...
            this.HasRequired            (x => x.Anbar_Az)
                .WithMany               (x => x.FactorHead_Az)
                .HasForeignKey          (x => x.FK_Anbar_Az)
                ;

            this.HasOptional            (x => x.Anbar_Be)
                .WithMany               (x => x.FactorHead_Be)
                .HasForeignKey          (x => x.FK_Anbar_Be)
                ;
