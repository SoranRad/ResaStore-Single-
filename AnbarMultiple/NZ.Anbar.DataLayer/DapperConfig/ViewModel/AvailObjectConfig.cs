...
LEFT OUTER JOIN (
                    ...
                    tar.FK_Kala, 
                    SUM(	CASE 
			                    WHEN (tat.kind>=11 AND tat.kind<50 AND tat.FK_Anbar_Az = @Anbar	) OR (tat.kind = 55 AND tat.FK_Anbar_Be = @Anbar)  THEN tar.meqdar
                    ...
                    AND     (tat.FK_Anbar_Az = @Anbar OR (tat.FK_Anbar_Be= @Anbar AND tat.kind= 55))
