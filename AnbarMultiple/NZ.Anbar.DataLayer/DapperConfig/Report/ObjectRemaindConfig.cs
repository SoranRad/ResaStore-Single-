...
namespace NZ.Anbar.DataLayer.DapperConfig.Report
{
  ...
  SUM( CASE WHEN (tat.kind >= 12 AND tat.kind < 50      AND tat.FK_Anbar_Az = @Anbar) OR (tat.kind = 55 AND tat.FK_Anbar_Be = @Anbar)	THEN tar.meqdar ELSE 0 END)  AS Input,
  SUM( CASE WHEN tat.kind >= 50  AND tat.kind <= 100	AND tat.NoRemainEffect = 0 AND tat.FK_Anbar_Az = @Anbar THEN tar.meqdar ELSE 0 END)  AS Output,
  
  SUM( CASE WHEN    tat.kind  =  13				  		            THEN tar.Remain * tar.nerkh_2
            WHEN    (tat.kind  >= 11 AND tat.kind <  50 AND tat.FK_Anbar_Az = @Anbar) 
                 OR (tat.kind = 55 AND tat.FK_Anbar_Be = @Anbar)    THEN tar.Remain * tar.nerkh 
            ELSE 0 
        END) AS RemainMablaq
