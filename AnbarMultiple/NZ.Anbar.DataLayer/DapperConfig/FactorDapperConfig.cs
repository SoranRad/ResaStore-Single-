...
SELECT tat.ID ,
       tat.{nameof(FactorHead.FK_Anbar_Az)},
       tat.{nameof(FactorHead.FK_Anbar_Be)},       
       tat.{nameof(FactorHead.NoRemainEffect)},       
       tat.{nameof(FactorHead.FK_Tahvil_Girande)},
       tat.FK_Salmali ,
       tat.FK_User_Add ,
       tat.FK_AshXas_ID ,
       tat.FK_Kind_Frosh ,
...
WHERE tat.FK_Salmali=@Year AND tat.kind=@Kind AND tat.{nameof(FactorHead.FK_Anbar_Az)} = @Anbar 
";

        private static string CodeUnique    = $@"
SELECT COUNT(tat.Serial) FROM Anbar.tbl_Amaliat_Title AS tat
WHERE tat.FK_Salmali=@Year AND tat.kind=@Kind AND tat.Serial=@Serial AND tat.{nameof(FactorHead.FK_Anbar_Az)} = @Anbar
";
