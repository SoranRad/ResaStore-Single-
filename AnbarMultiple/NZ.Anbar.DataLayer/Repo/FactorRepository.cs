        ...
        public void                     Delete          (long ID)
        {
            using (var context= new StorageContext(ConnectionManager.Create(), false))
            {
                ...
                if (Item != null)
                {
                    ...
                    if (Item.kind == (byte)Enums.NzFactorKind.EnteqalBeynAnbar)
                    {
	                    foreach (FactorItem riz in Item.FactorItems.ToList())
		                    KardexProcess.DeleteEnteqal(context, Item, riz, listDelete);
                    }
                ...
                try
                {
                    using (var db = new StorageContext(ConnectionManager.Create(), false))
                    {
                        ...
                        if (Factor.ID == 0)
                        {
                            ...
                            {
	                            foreach (FactorItem item in Factor.FactorItems.ToList())
		                           KardexProcess.AddEnteqal(db, Factor, item,  new List<long>());
                            }
                        }
                        else
                        {
                            ...
                            if (Factor.kind == (byte)Enums.NzFactorKind.EnteqalBeynAnbar)
                            {
                                ...
                                {
                                    if (item.State == Enums.NzItemState.Deleted)
                                        KardexProcess.DeleteEnteqal(db, Factor, item, listDelete);
                                    else if (item.State == Enums.NzItemState.Modified)
                                        KardexProcess.UpdateEnteqal(db, Factor, item, listDelete);
                                    else if (item.State == Enums.NzItemState.AddedNew)
                                        KardexProcess.AddEnteqal(db, Factor, item, listDelete);
                                }
                            }
