...
{
    public partial class FormListStorage :Form 
    {
        #region Logging
        ...
        private FormStorage     _FormItem;
        #endregion
        #region Constructor
        public FormListStorage()
        {
            InitializeComponent();
        ...
        #endregion
        #region Methods
        private void    Create_Form            (Storage Item)
        {
            _FormItem?.Dispose();

            _FormItem               = new FormStorage(_Manager, Item);
        ...
        private void    Frm_MS_Do_Save         (object sender, EventArgs e)
        {
            ...
            var row = ms_Grid.GetRows()
                .SingleOrDefault(x => (x.DataRow as Storage).ID == id);
            if (row == null) return;
        ...
        private void    RefreshGrid            ()
        {
            try
            {
                ms_Grid.DataSource = _Manager.GetList<Storage>
        ...
        private bool    HaveCircular           (Storage Item)
        {
            if (_Manager
        ...
        {
            var Row = ms_Grid.CurrentRow.DataRow as Storage;
            if (e.Column.Key == "E")
            {
                Create_Form(Row);
                _FormItem.Show(this);
