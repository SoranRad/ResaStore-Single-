...
namespace Nz.Anbar.WinForms.Alarm
{
    public partial class StorageAlarm : UserControl
    {
        ...
        private IEnumerable<PointOrder> _List= new List<PointOrder>();
        ...
        public void         RefreshList     ()
        {
            try
            {
	            ...
	            var storages   = manager.GetList<Storage>(new
	            {
		            Year = SystemConstant.ActiveYear.Salmali
	            }); 
	            IEnumerable<PointOrder> list = new List<PointOrder>();

                foreach (var storage in storages)
                {
	                list       = Mgr.GetReport<PointOrder>
	                (new
		                {
			                Year = SystemConstant.ActiveYear.Salmali,
                            Anbar = storage.ID,
