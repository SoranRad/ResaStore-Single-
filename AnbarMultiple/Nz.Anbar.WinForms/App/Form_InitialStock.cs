...
namespace Nz.Anbar.WinForms.App
{
    public partial class Form_InitialStock : Form
    {
        ...
        private readonly    short? _anbarId;

        #endregion
        #region Constructor
        public Form_InitialStock(long? RowSelect = null,short? AnbarId = null)
        {
            InitializeComponent();
        ...
        {
	        if(_anbarId>0)
		        NzAnbar.MS_Set_Select(_anbarId.Value);
        ...
        private void Save                           ()
        {
            try
            {
	            if ((NzAnbar.MS_Get_Selected() as Storage) == null)
	            {
		            MS_Message.Show("انـبار را انتـخاب کنیـد");
		            NzAnbar.Focus();
                    return;
	            }
                if (_Factor.ID == 0)
