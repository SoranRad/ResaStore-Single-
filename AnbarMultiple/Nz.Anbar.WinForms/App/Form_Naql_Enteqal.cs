...
{
	public partial class Form_Naql_Enteqal : Form_Mother_IRANSans
	{
		#region Logging
		...
		private long                _ID;
		private int                 _Serial;
		private Enums.NzFactorKind  _Kind = Enums.NzFactorKind.EnteqalBeynAnbar; 
		#endregion
		public Form_Naql_Enteqal                    (long ID = 0)
		{
			InitializeComponent(); 
			_ID         = ID;
        ...
        private void LoadItem                       ()
        {
            try
            {
                ...
                nzStorageBe.MS_Set_Select(_Factor.FK_Anbar_Be);
                NzAnbar.Enabled         = false;
                nzStorageBe.Enabled     = false;
        ...
        private void Save                           ()
        {
            if (_Factor.ID == 0)
            {
                ...
                _Factor.FK_Anbar_Be = (nzStorageBe.MS_Get_Selected() as Storage).ID; 
            }
        ...
        private bool IsOK                           ()
        {
            ...
            if (nzStorageBe.MS_Get_Selected() == null)
            {
	            nzStorageBe.Focus();
	            mS_Notify1.Show(nzStorageBe);
	            return false;
            }

            var az = (NzAnbar.MS_Get_Selected() as Storage).ID;
            var be = (nzStorageBe.MS_Get_Selected() as Storage).ID;

            if (az == be)
            {
	            MS_Message.Show("کاربر گرامی ، دقت فرمایید انبار مقصد و انبار مبدا نباید یکسان باشد.");
	            mS_Notify1.Show(nzStorageBe);
	            return false;
            }
