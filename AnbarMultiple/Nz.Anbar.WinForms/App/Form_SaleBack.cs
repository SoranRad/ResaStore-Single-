...
{
    public partial class Form_SaleBack : Form_Mother_IRANSans,IForm_Editor
    {
        #region Logging
        ...
        #endregion
        #region Constructor
        public Form_SaleBack                        (long ID = 0,Enums.NzFactorKind Kind = Enums.NzFactorKind.BargshtFrosh)
        {
            InitializeComponent();
        ...
        private void RemoveUnSavedRow               ()
        {
            try
            {
                ...
                _Factor
                    .FactorItems
                    .Where(x => x.mablaq > 0 && x.nerkh == 0 && x.State != Enums.NzItemState.Deleted)
                    .MSZ_ForEach(x =>
                    {
        ...
        private decimal GetLastParice               (int Code_Kala)
        {
            var Mgr     = new ReportManager();
            return Mgr.GetItem<PriceList>
            (new
                {
                    Year        = SystemConstant.ActiveYear.Salmali,
                    FK_Kala     = Code_Kala,
                    Kind1       = (byte) Enums.NzFactorKind.AvalDore,
                    Kind2       = (byte) Enums.NzFactorKind.Xarid,
                }
                , null
            )?.LastPrice??0;
        }
        ...
        private void ms_grid_UpdatingRecord         (object sender, CancelEventArgs e)
        {
            try
            {
                ...
                if (key?.meqdar == 0 || key?.FK_Kala == 0 || key?.nerkh==0 || key?.nerkh_2==0)
                {
        ...
        private void NzObjectPopup1OnNzSelectObject     (object sender, EventArgs eventArgs)
        {
...

            {
                ...
                Current.Cells["nerkh_2"].Value = GetLastParice(obj.Code);

                RefreshPrice();
            }

            NzGrid.CurrentColumn = NzGrid.RootTable.Columns["nerkh_2"];
        }
        ...
        private void NzPayment_Click                    (object sender, EventArgs e)
        {
            ...
            if (_Factor.ID > 0)
            {
                ...
                //var frm = Form_Factory._Form_Factory_Xazaneh.GetFormForEdit(kind);
		...
		private void NsChooseFactor_Click               (object sender, EventArgs e)
		{
			...
			if (anbar == null)
			{
				MS_Message.Show("انـبار را انتخاب کنید");
                NzAnbar.ShowMenu();
                return;
			}

			var frm = new FormSelectFactor(anbar.ID);
