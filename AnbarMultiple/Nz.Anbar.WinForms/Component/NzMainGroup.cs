...
{
    public partial class NzMainGroup : MS_TextBox_ADC
    {
        public NzMainGroup()
        {
            InitializeComponent();
        ...
        public override void    MS_Set_Select       (object             Item_to_Select)
        {
            ...
            {
                var item = Item_to_Select as MainGroup;
            ...
            else if (Item_to_Select is short)
            {
                if (_Grid != null)
                {
                    ...
                    var item = _Grid.MS_Get_Selected() as MainGroup;
                    _Selected_Item = item;
                    if (item == null)
                        this.Text = "";
                    else
        ...
        private void            NzList_Selected     (On_Item_Selected   e)
        {
            ...
            if (row != null)
            {
                var item = row.DataRow as MainGroup;
                Text = item.Code + @" ) " + item.title.Trim();
                _Selected_Item = item;
                SelectAll();
            }
