...
{
	public partial class NzStorage : MS_TextBox_ADC
	{
		public NzStorage()
		{
			InitializeComponent();
		...
		public override void    MS_Set_Select       (object             Item_to_Select)
		{
			...
			{
				var item = Item_to_Select as Storage;
			...
			else if (Item_to_Select is short)
			{
				if (_Grid != null)
				{
					...
					var item = _Grid.MS_Get_Selected() as Storage;
					_Selected_Item = item;
					if (item == null)
						this.Text = "";
					else
		...
		private void            NzList_Selected     (On_Item_Selected   e)
		{
			...
			if (row != null)
			{
				var item = row.DataRow as Storage;
				Text = item.Code + @" ) " + item.Title.Trim();
				_Selected_Item = item;
				SelectAll();
			}
