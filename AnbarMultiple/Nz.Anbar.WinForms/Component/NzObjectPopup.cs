...
namespace Nz.Anbar.WinForms.Component
{
    public class NzObjectPopup : ToolStripDropDown
    {
        ...
        private ToolStripButton         NzAdd, NzRefresh, NzSelect , NzSearchMode;
        ...
        public void SetAnbarFilter(short AnbarId)
        {
	        Anbar = AnbarId;
	        NzRefreshOnClick(this,null);
        }
        ...
        protected override bool     ProcessCmdKey   (ref Message m, Keys keyData)
        {
                ...
                || keyData == Keys.F11)
            {
	            if (keyData == Keys.F11)
	            {
		            SearchMode = !SearchMode;
		            NzSearchMode.Checked = SearchMode;
		            NzFilterBoxOnTextChanged(null, null);
		            return true;
	            }
        ...
        {
	        var list = new List<AvailObject>();

	        //if (Anbar != null)
		       // list = _List
			      //  .Where(x => x.FK_Anbar_Az == Anbar || x.kind == (byte)Enums.NzObjectKind.Service)
			      //  .ToList();
	        //else
		        list = _List;

	        if (_FactorMabnaId > 0 && _FactorItemsIDList.Any())
	        {
		        list =
		        (   from i in _List
			        join j in _FactorItemsIDList on i.Code equals j.Code
