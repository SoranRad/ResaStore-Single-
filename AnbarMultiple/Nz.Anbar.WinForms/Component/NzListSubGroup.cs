...
{
    public partial class NzListSubGroup : MS_Base_GridPopup
    {
        #region Fields
        private Manager                     _Manager;
        private IEnumerable<SubGroup>       _ListAccounts;
        private Control                     _DropMenu;
        #endregion

        public                  NzListSubGroup  ()
        {
            InitializeComponent();
        ...
        public  override void   MS_Set_Select   (object Item_to_Select)
        {
            ...
            {
                var Tag_Row         = Item_to_Select as SubGroup;
                var row             = ms_grid.GetDataRows()
                                            .FirstOrDefault
                                            (x => ((SubGroup)x.DataRow).ID == Tag_Row.ID);
                if (row != null)
                {
                    ms_grid.MoveTo(row);
        ...
        private void    NzAdd          (object sender, EventArgs eventArgs)
        {
            ...
            p?.Close();
            var frm = new FormSubGroup(_Manager);
