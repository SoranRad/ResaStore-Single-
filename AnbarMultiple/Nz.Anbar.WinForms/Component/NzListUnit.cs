...
{
    public partial class NzListUnit : MS_Base_GridPopup
    {
        #region Fields
        private Manager                     _Manager;
        private IEnumerable<Unit>           _ListAccounts;
        ...
        public  override void   MS_Set_Select   (object Item_to_Select)
        {
            ...
            {
                var Tag_Row         = Item_to_Select as Unit;
                var row             = ms_grid.GetDataRows()
                                            .FirstOrDefault
                                            (x => ((Unit)x.DataRow).ID == Tag_Row.ID);
                if (row != null)
                {
                    ms_grid.MoveTo(row);
        ...
        {
            _ListAccounts   = _Manager.GetList<Unit>(new
            {
                Year = SystemConstant.ActiveYear.Salmali
            });
            RefreshControl();
