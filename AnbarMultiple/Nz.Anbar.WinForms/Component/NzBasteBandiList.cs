...
{
    public partial class NzBasteBandiList : MS_Base_GridPopup
    {
        #region Fields
        private Manager _Manager;
        private IEnumerable<BasteBandi> _List;
        private Control _DropMenu;
        #endregion
        public NzBasteBandiList()
        {
            InitializeComponent();
        ...
        {
            _List = _Manager.GetList<BasteBandi>();
        ...
        public override void MS_Set_Select(object Item_to_Select)
        {
            ...
            {
                var Tag_Row = Item_to_Select as BasteBandi;
                var row = ms_grid.GetDataRows()
                                            .FirstOrDefault
                                            (x => ((BasteBandi)x.DataRow).ID == Tag_Row.ID);
                if (row != null)
                {
                    ms_grid.MoveTo(row);
