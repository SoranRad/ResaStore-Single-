            ...
            switch (_Level)
            {
                ...
                case 4:
                    if (NzGrid.CurrentRow.DataRow is CircularObject row)
                    {
                        ...
                        else if (kind == Enums.NzFactorKind.HavaleXoruj || kind == Enums.NzFactorKind.ResidVorud)
	                        new Form_Transfer(row.IDTitle, kind).ShowDialog();
                        else if (kind == Enums.NzFactorKind.AvalDore)
                        {
	                        var frm = new Form_InitialStock(row.ID,row.FK_Anbar_Az);
	                        frm.MdiParent = this.MdiParent;
	                        frm.Show();
                        }
                        else if (kind == Enums.NzFactorKind.EnteqalBeynAnbar)
	                        new Form_Naql_Enteqal(row.IDTitle).ShowDialog(this);
                        else if(kind==Enums.NzFactorKind.BargshtFrosh)
	                        new Form_SaleBack(row.IDTitle, kind).ShowDialog(this);
                        else
	                        new Form_Purchase(row.IDTitle, kind).ShowDialog(this);
                    }
                    break;
            }
        }
