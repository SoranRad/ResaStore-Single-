...
{
    public partial class NzListCity : MS_Base_GridPopup
    {
        #region Fields
        private Manager                     _Manager;
        private IEnumerable<City>           _List;
        ...
        public  override void   MS_Set_Select   (object Item_to_Select)
        {
            ...
            else if (Item_to_Select is City)
            {
                var Tag_Row         = Item_to_Select as City;
                var row             = ms_grid.GetDataRows()
                                            .FirstOrDefault
                                            (x => ((City)x.DataRow).ID == Tag_Row.ID);
                if (row != null)
                {
                    ms_grid.MoveTo(row);
        ...
        private void NzAdd      (object sender, EventArgs eventArgs)
        {
            ...
            p?.Close();
            var frm = new Form_City(_Manager, null);
