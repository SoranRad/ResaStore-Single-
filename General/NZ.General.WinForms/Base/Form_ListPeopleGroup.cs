...
{
    public partial class FormListpeopleGroup : Form
    {
        #region Logging
        ...
        #endregion
        #region Fields
        private Form_PeopleGroup        _FormPeopleGroup;
        private Manager                 _Manager;
        #endregion
        #region Consractor
        public FormListpeopleGroup()
        {
            InitializeComponent();
        ...
        #endregion
        #region Methods
        private void Create_Form                    (PeopleGroup PeopleGroup)
        {
            _FormPeopleGroup?.Dispose();

            _FormPeopleGroup                = new Form_PeopleGroup(_Manager, PeopleGroup);
            _FormPeopleGroup.MS_Do_Save     += Frm_MS_Do_Save;
            _FormPeopleGroup.FormClosed     += Frm_FormClosed;
        }
        private void Frm_MS_Do_Save                 (object sender, EventArgs e)
        {
            ...
            var row         = mS_GridX1.GetRows()
                .SingleOrDefault(x => (x.DataRow as PeopleGroup).ID == id);
            if (row == null) return;
        ...
        private bool HaveCircular                   (PeopleGroup Item)
        {
            if (_Manager
                .HaveCircular<PeopleGroup>
                (new
                {
                    Code=Item.ID
                })
