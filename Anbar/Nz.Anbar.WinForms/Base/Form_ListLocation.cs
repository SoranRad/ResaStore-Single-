...
{
    public partial class Form_ListLocation : Form
    {
        #region Logging
        ...
        private Form_Locatio   _FormItem;
        #endregion
        public Form_ListLocation()
        {
            InitializeComponent();
        ...
        #region Methods
        private void Create_Form            (Location Item)
        {
            _FormItem?.Dispose();

            _FormItem = new Form_Locatio(_Manager,Item);
        ...
        private void Frm_MS_Do_Save         (object sender, EventArgs e)
        {
            ...
            var row = ms_Grid.GetRows()
                .SingleOrDefault(x => (x.DataRow as Location).ID == id);
            if (row == null) return;
        ...
        private void RefreshGrid            ()
        {
            try
            {
                ms_Grid.DataSource = _Manager.GetList<Location>
                    (new
                    {
        ...
        private bool HaveCircular           (Location Item)
        {
            if (_Manager
                        .HaveCircular<Location>
                            (new
                            {
                                Item.ID
                            })
        ...
        {
            var Row = ms_Grid.CurrentRow.DataRow as Location;
            if (e.Column.Key == "E")
            {
                Create_Form     (Row);
                _FormItem.Show  (this);
