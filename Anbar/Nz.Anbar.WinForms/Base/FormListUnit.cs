...
{
    public partial class FormListUnit : Form 
    {
        #region Logging
        ...
        #endregion
        #region Methods
        private void Create_Form    (Unit Item)
        {
            if (_FormItem != null)
                _FormItem.Dispose();

            _FormItem               = new FormUnit(_Manager, Item);
        ...
        private void Frm_MS_Do_Save (object sender, EventArgs e)
        {
            ...
            var row     = ms_Grid.GetRows()
                                 .SingleOrDefault(x => (x.DataRow as Unit).ID == id);
            if (row == null) return;
        ...
        private void RefreshGrid    ()
        {
            try
            {
                ms_Grid.DataSource = _Manager.GetList<Unit>()?.ToList();
            }
            catch (Exception ex)
            {
        ...
        private bool HaveCircular   (Unit Item)
        {
            if (_Manager
                        .HaveCircular<Unit>
                            (new
        ...
        {
            var Row = ms_Grid.CurrentRow.DataRow as Unit;
            if (e.Column.Key == "E")
            {
                Create_Form(Row);
                _FormItem.Show(this);
        ...
        private void    FormListUnit_PreviewKeyDown     (object sender, PreviewKeyDownEventArgs e)
        {
