...
{
    public partial class FormSaleKind : Form_Mother_IRANSans
    {
        #region Logging
        ...
        private SaleKind            _Item;
        private bool                _Is_Edit = false;
        public event EventHandler   MS_Do_Save;
        #endregion
        #region Constructor
        public FormSaleKind     (Manager Manager, SaleKind Row = null)
        {
            InitializeComponent();
        ...
        private void GenerateKindValue()
        {
            NzKind.Items.Clear();
            foreach (Enums.NzSalingKind kind in Enum.GetValues(typeof(Enums.NzSalingKind)))
            {
                NzKind.Items.Add(new UIComboBoxItem()
                {
                    Text = kind.NzTostring(),
                    DataRow = kind,
                    Value = kind,
                });
            }
