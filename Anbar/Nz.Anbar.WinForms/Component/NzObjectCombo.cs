...
{
    public partial class NzObjectCombo : MS_TextBox_ADC
    {
        public NzObjectCombo()
        {
            InitializeComponent();
        ...
        public  override void   MS_Set_Select       (object Item_to_Select)
        {
            ...
            {
                var item = Item_to_Select as NzObject;
            ...
            else if (Item_to_Select is int)
            {
                if (_Grid != null)
                {
                    ...
                    var item = _Grid.MS_Get_Selected() as NzObject;
                    _Selected_Item = item;
                    if (item == null)
                        this.Text = "";
                    else
        ...
        private void            NzList_Selected     (On_Item_Selected e)
        {
            ...
            if (row != null)
            {
                var item = row.DataRow as NzObject;
                Text = item.Code + @" ) " + item.title.Trim();
                _Selected_Item = item;
                SelectAll();
            }
